...
public:
  HKAttestationAuth(std::vector<hkIssuer_t> &issuers, DigitalKeySecureContext &context, const std::function<bool(uint8_t*, uint8_t, uint8_t*, uint16_t*, bool)>& nfc) : issuers(issuers), nfc(nfc), DKSContext(context){/* esp_log_level_set(TAG, ESP_LOG_VERBOSE); */};
